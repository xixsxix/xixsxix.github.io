---
title: European Cafe Lounge · Black Heart Ruffle Dress
date: '"2026-05-17 22:46:02 +0900"'
categories:
  - SORAprompt
tags:
  - sora_post
image: /assets/sora/sora_european_cafe_black_heart_01_04.jpg
permalink: /sora_ECLBHR01/
hide_image: false
pin: false
published: true
---

# Sora · European Cafe Lounge · Black Heart Ruffle Dress

## 01. 촬영 개요

- 캐릭터: Sora
- 촬영 ID: SORA_EUROPEAN_CAFE_BLACK_HEART_01
- 환경: ENV_EUROPEAN_CAFE_LOUNGE
- 의상: OUTFIT_BLACK_HEART_RUFFLE_BOW_MINI_DRESS
- 조명: ASHROSE
- 프레임: 16:9 horizontal
- 촬영 타입: horizontal_scene_set
- 총 샷 수: 7
- 촬영 목적:  
  유럽풍 카페 라운지의 따뜻한 창가 무드 안에서 Sora의 얼굴 동일성과 의상 정체성을 함께 보여주는 가로 장면 세트.  
  블로그 포스트, YouTube 이미지 영상, Instagram 단독 이미지 재사용을 모두 고려한 구성.

---

## 02. 사용 모듈

### CORE

- SORA ACTIVE CORE
- SORA FACE EXECUTION LOCK
- SORA FACE DNA LOCK
- SORA BODY CORE
- SORA HAIR PHYSICS SYSTEM

### RENDER

- SORA 3D ANI LOCK

### ENVIRONMENT

```markdown
ENV_EUROPEAN_CAFE_LOUNGE
```

### OUTFIT

```markdown
OUTFIT_BLACK_HEART_RUFFLE_BOW_MINI_DRESS
```

### LIGHT

```markdown
ASHROSE
```

---

## 03. 환경 정보

- 조용한 유럽풍 카페 라운지 실내
- 창가가 주요 공간 앵커로 작동하는 구조
- 원형 테이블, 카페 의자, 화병, 커피잔이 프레임 밀도를 형성
- 따뜻한 아이보리 톤과 부드러운 실내 깊이감 유지
- 붐비지 않는 정적인 실내 분위기
- Sora를 돋보이게 하는 restrained cinematic indoor mood 중심

---

## 04. 의상 정보

- 블랙 미니 드레스 베이스
- 화이트 오프숄더 러플 네크라인
- 하트 패턴이 들어간 시스루 퍼프 슬리브
- 중앙의 대형 블랙 체스트 보우
- 슬림한 상체와 부드럽게 퍼지는 미니 스커트 실루엣
- 전체적으로 로맨틱하고 단정한 feminine styling 유지

---

## 05. 샷 기록

## Shot 01 — 창가 인트로 미디엄 샷

### 목적

- 카페 창가 분위기와 함께 Sora의 상반신 인상을 안정적으로 소개하는 오프닝 컷
- 얼굴과 의상 상부 디테일을 동시에 보여주는 기본 소개 컷

### 이미지

![Shot 01](/assets/sora/sora_european_cafe_black_heart_01_01.jpg)

### 사용 커맨드

```markdown
[16:9 horizontal frame]
[medium shot by the cafe window]
[quiet European cafe lounge]
[soft window light on face and shoulders]
[Sora wearing OUTFIT_BLACK_HEART_RUFFLE_BOW_MINI_DRESS]
[upper body clearly visible]
[one arm resting naturally near the table]
[soft off-lens gaze]
[calm restrained expression]
[natural stillness]
```

---

## Shot 02 — 창가 에디토리얼 시팅 샷

### 목적

- 카페 창가와 도시 배경, 실내 깊이감을 함께 담는 대표 장면 컷
- 블로그 본문과 영상 전개에서 분위기 전환용으로 사용하기 좋은 메인 시팅 샷

### 이미지

![Shot 02](/assets/sora/sora_european_cafe_black_heart_01_02.jpg)

### 사용 커맨드

```markdown
[16:9 horizontal frame]
[editorial seated shot beside a tall cafe window]
[quiet European cafe lounge]
[window-side table composition]
[Sora wearing OUTFIT_BLACK_HEART_RUFFLE_BOW_MINI_DRESS]
[softly flared skirt partially visible]
[cafe cup and flower details in the frame]
[Sora looking gently outside the window]
[calm reflective expression]
[natural seated posture]
```

---

## Shot 03 — 3/4 전신 시팅 샷

### 목적

- 의상 실루엣과 착석 포즈를 함께 확인하는 3/4 전신 컷
- 미니 드레스의 길이감, 테이블 구조, 실내 공간감을 함께 보여주는 구성

### 이미지

![Shot 03](/assets/sora/sora_european_cafe_black_heart_01_03.jpg)

### 사용 커맨드

```markdown
[16:9 horizontal frame]
[three-quarter seated shot at a small round cafe table]
[quiet European cafe lounge]
[soft window light and warm interior depth]
[Sora wearing OUTFIT_BLACK_HEART_RUFFLE_BOW_MINI_DRESS]
[mini dress silhouette visible]
[one arm resting lightly near the table]
[soft off-lens gaze]
[calm restrained expression]
[natural stillness]
```

---

## Shot 04 — 의상 디테일 샷

### 목적

- 블랙 보우, 러플 네크라인, 시스루 하트 패턴 디테일을 선명하게 보여주는 디테일 컷
- 의상 소개 파트와 썸네일 보조 이미지용으로 적합한 컷

### 이미지

![Shot 04](/assets/sora/sora_european_cafe_black_heart_01_04.jpg)

### 사용 커맨드

```markdown
[16:9 horizontal frame]
[outfit detail shot from chest to waist]
[quiet European cafe lounge]
[soft window light]
[large black chest bow centered and clearly visible]
[white off-shoulder ruffle neckline clearly visible]
[black heart pattern readable on sheer puff sleeves]
[delicate necklace visible]
[black fitted bodice visible]
[soft fabric texture readable]
[calm cinematic detail composition]
```

---

## Shot 05 — 얼굴 클로즈업 샷

### 목적

- Sora의 얼굴 동일성, 눈의 분위기, 부드러운 표정 안정성을 강조하는 클로즈업 컷
- 블로그 본문 중간 포인트 이미지 및 Instagram 단독 활용에 적합

### 이미지

![Shot 05](/assets/sora/sora_european_cafe_black_heart_01_05.jpg)

### 사용 커맨드

```markdown
[16:9 horizontal frame]
[intimate face close-up]
[quiet European cafe lounge background softly blurred]
[soft warm window light]
[Sora wearing OUTFIT_BLACK_HEART_RUFFLE_BOW_MINI_DRESS]
[face fills most of the frame]
[eyes softly focused near the lens]
[gentle but not smiling expression]
[relaxed neutral mouth]
[restrained affectionate gaze]
[natural intimate mood]
```

---

## Shot 06 — 전신 환경 샷

### 목적

- 카페 실내 구조와 전신 실루엣을 함께 보여주는 장면형 컷
- 블로그 포스트의 후반부와 YouTube 영상에서 공간 확장 컷으로 사용하기 좋은 이미지

### 이미지

![Shot 06](/assets/sora/sora_european_cafe_black_heart_01_06.jpg)

### 사용 커맨드

```markdown
[16:9 horizontal frame]
[full body shot beside the cafe window]
[quiet European cafe lounge]
[soft window light and warm interior depth]
[Sora wearing OUTFIT_BLACK_HEART_RUFFLE_BOW_MINI_DRESS]
[full dress silhouette clearly visible]
[black ankle-strap heels visible]
[small shoulder bag visible]
[one hand lightly touching the window frame]
[soft off-lens gaze]
[natural stillness]
```

---

## Shot 07 — 포스팅 대표 시팅 샷

### 목적

- 전체 세트의 마무리이자 대표 이미지 후보로 사용할 수 있는 안정적인 시팅 컷
- 창가, 원형 테이블, 커피잔, 화병, 의상 상부 실루엣이 균형 있게 모인 커버 후보 컷

### 이미지

![Shot 07](/assets/sora/sora_european_cafe_black_heart_01_07.jpg)

### 사용 커맨드

```markdown
[16:9 horizontal frame]
[medium shot seated at a small round cafe table]
[quiet European cafe lounge]
[soft warm window light]
[Sora wearing OUTFIT_BLACK_HEART_RUFFLE_BOW_MINI_DRESS]
[upper body and skirt visible]
[one hand resting naturally on the table]
[other hand relaxed on her lap]
[slight body angle]
[soft observational gaze toward the window]
[calm restrained expression]
[natural seated posture]
```