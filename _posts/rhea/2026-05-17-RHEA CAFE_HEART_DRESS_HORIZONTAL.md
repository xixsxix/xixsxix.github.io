---
title: European Cafe Black Heart Dress Horizontal
date: 2026-05-17
environment: ENV_EUROPEAN_CAFE_LOUNGE
outfit: OUTFIT_BLACK_HEART_RUFFLE_BOW_MINI_DRESS
light_state: ASHROSE
frame: 16:9 horizontal
shoot_type: horizontal_scene_set
shot_count: 6
categories:
  - RHEAprompt
tags:
  - rhea-post
  - horizontal-scene-set
  - european-cafe
  - black-heart-ruffle-dress
image:
  path: //assets/rhea/european_cafe_black_heart_dress_horizontal_cover.jpg
  alt: rhea in a quiet European cafe wearing a black heart ruffle bow mini dress
permalink: /rhea/cafe-heart-dress-horizontal/
hide_image: false
pin: false
published: true
---

# European Cafe, Black Heart Dress Horizontal

## 01. 촬영 개요

- 캐릭터: rhea
- 촬영 ID: `rhea_EUROPEAN_CAFE_BLACK_HEART_DRESS_HORIZONTAL_01`
- 환경: `ENV_EUROPEAN_CAFE_LOUNGE`
- 의상: `OUTFIT_BLACK_HEART_RUFFLE_BOW_MINI_DRESS`
- 조명: `ASHROSE`
- 프레임: `16:9 horizontal`
- 촬영 타입: `horizontal_scene_set`
- 총 샷 수: 6
- 촬영 목적:
  - 유럽풍 카페 라운지 환경에서 rhea의 로맨틱한 블랙 미니드레스 의상을 가로형 영상 컷으로 검증한다.
  - 블로그 포스트, YouTube 영상, 썸네일, Instagram 이미지로 재사용 가능한 컷 구성을 확보한다.
  - 의상 디테일, 얼굴 클로즈업, 착석 장면, 전신성 장면, 환경 소개 컷을 한 세트로 구성한다.

---

## 02. 사용 모듈

### CORE

- `rhea ACTIVE CORE`
- `rhea FACE DNA LOCK`
- `rhea BODY STANDARD 01`
- `HAIR_STYLE_OBSIDIAN_STRAIGHT_BASELINE`
- `rhea HAIR PHYSICS SYSTEM`

### RENDER

- `rhea 3D ANI LOCK`

### ENVIRONMENT

```markdown
ENV_EUROPEAN_CAFE_LOUNGE
```

### OUTFIT

```markdown
OUTFIT_BLACK_HEART_RUFFLE_BOW_MINI_DRESS
```

### LIGHT

```markdown
ASHROSE
```

---

## 03. 환경 정보

- 조용한 유럽풍 카페 라운지.
- 큰 창가, 작은 원형 카페 테이블, 의자, 꽃 장식, 부드러운 실내 벽면 장식이 중심 구조다.
- 공간은 화려한 호텔이나 궁전이 아니라, 로맨틱하지만 절제된 카페 실내 분위기를 유지한다.
- 창가 조명과 실내 깊이를 활용해 16:9 가로 프레임에서도 빈 공간 없이 장면 밀도를 만든다.
- 전경에는 꽃, 테이블, 컵, 창틀을 배치하고, 중경에는 rhea, 후경에는 카페 인테리어와 의자, 벽면 장식을 둔다.

---

## 04. 의상 정보

- 블랙 미니드레스와 화이트 오프숄더 러플 블라우스 효과가 결합된 시그니처 의상.
- 중심 디테일은 큰 블랙 가슴 리본, 화이트 러플 넥라인, 하트 패턴 시스루 퍼프 소매다.
- 실루엣은 상체가 피트되고 허리가 좁으며, 스커트는 짧고 부드럽게 퍼지는 A라인 미니드레스 구조다.
- 소재는 부드러운 블랙 드레스 원단과 가벼운 화이트 시폰 계열 러플 소재가 대비된다.
- 액세서리는 섬세한 목걸이, 귀걸이, 작은 블랙 숄더백 정도로 제한한다.

---

# 05. 샷 기록

## Shot 01 — 창가 웨이스트업 소개 샷

### 목적

- 환경과 의상 핵심 디테일을 동시에 보여주는 첫 장면.
- 큰 리본, 러플 넥라인, 하트 패턴을 블로그 본문 초반에서 명확히 전달한다.
- YouTube 영상의 부드러운 도입 컷으로 사용 가능하다.

### 이미지

![Shot 01](/assets/rhea/european_cafe_black_heart_dress_horizontal_01.jpg)

### 사용 커맨드

```markdown
[16:9 horizontal frame]
[waist-up shot beside a cafe window]
[quiet European cafe lounge]
[soft window light from the side]
[rhea wearing OUTFIT_BLACK_HEART_RUFFLE_BOW_MINI_DRESS]
[large black chest bow clearly visible]
[white off-shoulder ruffle neckline visible]
[black heart pattern readable]
[one hand lightly resting near the table]
[soft off-lens gaze]
[calm tender expression]
[natural stillness]
```

---

## Shot 02 — 카페 테이블 착석 미디엄 샷

### 목적

- 정적인 의상 소개에서 벗어나, 실제 카페 장면 안에 rhea가 머무는 느낌을 만든다.
- 착석 포즈를 통해 YouTube 영상 중간부의 호흡을 안정시킨다.
- 테이블, 창가, 손 위치를 활용해 자연스러운 생활 장면을 확보한다.

### 이미지

![Shot 02](/assets/rhea/european_cafe_black_heart_dress_horizontal_02.jpg)

### 사용 커맨드

```markdown
[16:9 horizontal frame]
[medium shot seated at a small round cafe table]
[quiet European cafe lounge]
[soft warm window light]
[rhea wearing OUTFIT_BLACK_HEART_RUFFLE_BOW_MINI_DRESS]
[upper body and skirt visible]
[one hand resting naturally on the table]
[other hand relaxed on her lap]
[slight body angle]
[soft observational gaze toward the window]
[calm restrained expression]
[natural seated posture]
```

---

## Shot 03 — 창가 3/4 전신 샷

### 목적

- 의상의 전체 실루엣과 rhea의 바디 라인을 확인하는 컷.
- 미니드레스 길이, 스커트 볼륨, 힐, 숄더백까지 한 번에 보여준다.
- 블로그 본문에서 의상 전체 구조 설명용으로 적합하다.

### 이미지

![Shot 03](/assets/rhea/european_cafe_black_heart_dress_horizontal_03.jpg)

### 사용 커맨드

```markdown
[16:9 horizontal frame]
[three-quarter full body shot beside a tall cafe window]
[quiet European cafe lounge]
[soft window light and warm interior depth]
[rhea wearing OUTFIT_BLACK_HEART_RUFFLE_BOW_MINI_DRESS]
[mini dress silhouette clearly visible]
[softly flared skirt visible]
[black ankle-strap heels visible]
[delicate shoulder bag visible]
[one foot slightly forward]
[subtle asymmetrical weight balance]
[soft off-lens gaze]
[natural stillness]
```

---

## Shot 04 — 친밀한 얼굴 클로즈업

### 목적

- rhea의 얼굴 동일성과 감정 톤을 확인하는 핵심 컷.
- 썸네일, Instagram, YouTube 영상의 감정 포인트로 사용 가능하다.
- 과한 미소 없이 다정하지만 절제된 시선을 유지한다.

### 이미지

![Shot 04](/assets/rhea/european_cafe_black_heart_dress_horizontal_04.jpg)

### 사용 커맨드

```markdown
[16:9 horizontal frame]
[intimate face close-up]
[quiet European cafe lounge background softly blurred]
[soft warm window light]
[rhea wearing OUTFIT_BLACK_HEART_RUFFLE_BOW_MINI_DRESS]
[face fills most of the frame]
[slight 15-degree face angle]
[eyes softly focused near the lens]
[gentle but not smiling expression]
[relaxed neutral mouth]
[restrained affectionate gaze]
[natural intimate mood]
```

---

## Shot 05 — 의상 디테일 샷

### 목적

- 블랙 리본, 러플 넥라인, 하트 패턴, 시스루 소매를 기록하는 디테일 컷.
- 의상 문서와 블로그 포스트에서 가장 실용적인 설명 컷이다.
- YouTube 영상에서는 짧은 인서트 컷으로 활용하기 좋다.

### 이미지

![Shot 05](/assets/rhea/european_cafe_black_heart_dress_horizontal_05.jpg)

### 사용 커맨드

```markdown
[16:9 horizontal frame]
[outfit detail shot from chest to waist]
[quiet European cafe lounge]
[soft warm window light]
[large black chest bow centered and clearly visible]
[white off-shoulder ruffle neckline clearly visible]
[black heart pattern readable on sheer puff sleeves]
[delicate necklace visible]
[black fitted bodice visible]
[soft fabric texture readable]
[no face crop requirement]
[calm cinematic detail composition]
```

---

## Shot 06 — 카페 창가 시네마틱 미디엄 샷

### 목적

- 이번 촬영 세트의 분위기를 가장 안정적으로 정리하는 대표 후보 컷.
- 환경, 의상, 얼굴, 조명, 전경 소품을 모두 포함한다.
- 블로그 커버 또는 YouTube 썸네일 후보로 사용 가능하다.

### 이미지

![Shot 06](/assets/rhea/european_cafe_black_heart_dress_horizontal_06.jpg)

### 사용 커맨드

```markdown
[16:9 horizontal frame]
[cinematic medium shot beside a cafe window]
[quiet European cafe lounge]
[rhea wearing OUTFIT_BLACK_HEART_RUFFLE_BOW_MINI_DRESS]
[soft window light on face and shoulders]
[European cafe interior depth behind her]
[flowers and small cafe table in foreground]
[rhea looking gently out the window]
[calm reflective expression]
[natural stillness]
[restrained cinematic atmosphere]
```

---

# 06. AI Creative Director 평가

## 전체 판정

- 결과: 사용 가능성이 높은 가로형 촬영 세트.
- 사용 가능성: 블로그 포스트, YouTube 영상, Instagram 이미지 모두에 사용 가능.
- 블로그 포스트 적합도: 높음.
- YouTube 영상 활용도: 높음.
- Instagram / Shorts 활용도: 중상.
- 핵심 성과:
  - 의상 핵심 디테일이 명확하다.
  - 유럽풍 카페 환경과 블랙 로맨틱 미니드레스의 궁합이 좋다.
  - 가로 프레임에서도 전경, 중경, 후경이 비교적 안정적으로 분리된다.
  - 얼굴 클로즈업과 의상 디테일 컷이 별도로 확보되어 포스트 구성이 좋다.

---

## 컷별 사용 판정

| Shot | 역할 | 판정 | 추천 용도 |
|---|---|---|---|
| Shot 01 | 창가 웨이스트업 소개 | 사용 가능 | 블로그 도입, 영상 오프닝 |
| Shot 02 | 착석 미디엄 | 사용 가능 | 영상 중간 컷, 블로그 흐름 컷 |
| Shot 03 | 3/4 전신 | 강력 사용 | 의상 전체 설명, 본문 핵심 컷 |
| Shot 04 | 얼굴 클로즈업 | 강력 사용 | 썸네일, Instagram, 감정 포인트 |
| Shot 05 | 의상 디테일 | 강력 사용 | 의상 설명, 인서트 컷 |
| Shot 06 | 시네마틱 미디엄 | 강력 사용 | 커버, YouTube 썸네일, 대표 이미지 |

---

## 대표 이미지 후보

- 1순위: Shot 06 — 카페 창가 시네마틱 미디엄 샷
- 2순위: Shot 04 — 친밀한 얼굴 클로즈업
- 3순위: Shot 03 — 창가 3/4 전신 샷

---

## 강점

- 가로형 프레임에서 카페 공간이 단순 배경이 아니라 장면으로 작동한다.
- 창가 조명, 꽃, 테이블, 카페 의자가 영상 컷으로 이어질 때 자연스러운 리듬을 만든다.
- `OUTFIT_BLACK_HEART_RUFFLE_BOW_MINI_DRESS`의 핵심 요소인 블랙 리본, 하트 패턴, 오프숄더 러플이 잘 드러난다.
- Shot 04와 Shot 06은 rhea의 감정 톤과 캐릭터성을 보여주는 대표 컷으로 가치가 높다.
- Shot 05는 의상 아카이브와 블로그 설명에 반드시 필요한 실용 컷이다.

---

## 아쉬운 점

- 총 6컷 구성이라 표준 7컷 세트보다 영상 편집 시 전환 컷이 하나 부족하다.
- 환경 와이드 샷이 별도로 없어서 카페 공간 자체를 보여주는 장면성이 조금 약할 수 있다.
- 다음 촬영에서는 같은 장소의 빈 테이블, 창가, 컵, 꽃을 활용한 무인 환경 컷 또는 넓은 와이드 컷을 추가하면 YouTube 편집이 더 좋아진다.
- 얼굴 캐논이 고정된 이후에는 Shot 04 같은 클로즈업에서 FACE DNA 유지 여부를 더 엄격하게 비교해야 한다.

---

## 다음 촬영 보정점

- Shot 07로 환경 와이드 샷 또는 썸네일 전용 컷을 추가한다.
- YouTube용으로는 컷마다 줌인 / 줌아웃 방향을 미리 정해서 생성 순서를 잡는다.
- 블로그 커버용 이미지는 얼굴, 의상, 환경이 모두 보이는 미디엄 샷을 우선한다.
- Instagram용은 얼굴 클로즈업과 의상 디테일 컷을 별도 후보로 관리한다.
- 다음 가로 촬영부터는 `horizontal_scene_set`을 6컷이 아니라 7컷 기준으로 유지한다.

---

# 07. 베스트 컷 정리

## Cover

- Shot 06 — 카페 창가 시네마틱 미디엄 샷
- 이유:
  - 환경, 얼굴, 의상, 전경 소품이 균형 있게 들어간다.
  - 블로그 대표 이미지와 YouTube 썸네일 양쪽에 가장 안정적이다.

## Main Body

- Shot 03 — 창가 3/4 전신 샷
- Shot 01 — 창가 웨이스트업 소개 샷
- Shot 02 — 카페 테이블 착석 미디엄 샷

## Detail

- Shot 05 — 의상 디테일 샷

## Video Cut

- Shot 01 — 오프닝
- Shot 02 — 착석 전환
- Shot 03 — 의상 전체 확인
- Shot 04 — 감정 클로즈업
- Shot 05 — 의상 인서트
- Shot 06 — 마무리 대표 컷

## Social Cut

- Shot 04 — Instagram 이미지 후보
- Shot 05 — 의상 소개용 이미지 후보
- Shot 06 — Shorts / Reels 썸네일 후보

---

# 08. 제작 메모

- 이번 촬영은 16:9 가로형 영상화를 염두에 둔 첫 번째 실전형 카페 세트로 볼 수 있다.
- 기존 세로샷보다 YouTube 영상 편집에 바로 연결하기 좋다.
- 한 장소 안에서 웨이스트업, 착석, 전신, 얼굴, 디테일, 대표 컷이 모두 확보되어 블로그 포스트 구조가 안정적이다.
- 다음부터는 촬영 계획 단계에서 영상 순서를 먼저 설계하고, 그 순서대로 이미지를 생성하는 것이 좋다.
- 6컷만으로도 사용 가능하지만, 장면 호흡을 위해 환경 와이드 컷 하나를 추가하면 완성도가 높아진다.

---

# 09. 재사용 가이드

## Blog

- Shot 06을 대표 이미지로 사용한다.
- Shot 03으로 의상 전체 구조를 설명한다.
- Shot 05로 리본, 러플, 하트 패턴을 설명한다.
- Shot 04로 rhea의 감정 톤과 얼굴 분위기를 강조한다.

## YouTube

- 권장 구성:
  - Shot 01: 도입
  - Shot 03: 의상 전체 소개
  - Shot 02: 착석 장면 전환
  - Shot 05: 의상 디테일 인서트
  - Shot 04: 얼굴 클로즈업
  - Shot 06: 마무리 대표 컷
- 각 컷에 느린 줌인 또는 좌우 패닝을 적용하면 정적인 이미지 세트도 영상처럼 보인다.
- Shot 05는 2초 내외의 짧은 디테일 컷으로 사용하는 것이 좋다.

## Instagram

- Shot 04는 단독 이미지 포스트에 적합하다.
- Shot 03은 의상 전신 소개용으로 적합하다.
- Shot 05는 의상 디테일 카드로 사용 가능하다.

## Shorts / Reels

- Shot 04 → Shot 05 → Shot 06 순서가 가장 짧고 강하다.
- 얼굴, 의상 디테일, 대표 분위기 순으로 이어지기 때문에 10초 내외의 숏폼 구성에 적합하다.
- BGM은 느린 피아노, 로맨틱 카페 재즈, 잔잔한 시네마틱 앰비언트 계열이 잘 맞는다.